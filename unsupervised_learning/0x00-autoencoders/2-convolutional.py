...
def autoencoder(input_dims, filters, latent_dims):
    ...
    encoded_layer = Conv2d(filters[0], (3, 3), activation="relu", padding="same")(encoder_input)
    encoded_layer = MaxPool((2, 2), padding="same")(encoded_layer)

    for filter in filters[1:]:
        encoded_layer = Conv2d(filter, (3, 3), padding="same", activation="relu")(encoded_layer)
        encoded_layer = MaxPool((2, 2), padding="same")(encoded_layer)

    decoded_layer = Conv2d(filters[-1], (3, 3), padding="same", activation="relu")(decoder_input)
    decoded_layer = UpSample((2, 2))(decoded_layer)

    for x, filter in enumerate(list(reversed(filters))[1:]):
        if x == len(filters) - 2:
            decoded_layer = Conv2d(filter, (3, 3), padding="valid", activation="relu")(decoded_layer)
        else:
            decoded_layer = Conv2d(filter, (3, 3), padding="same", activation="relu")(decoded_layer)
